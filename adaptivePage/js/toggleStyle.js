...
pp.getViewType = function () {
    //缩放会影响媒体查询的结果
    if ( !util.isMobile() ) {
        this.viewType = 1200;
        $(".style").attr( "href",  this.styleUrl[ "s1200" ] );
        this.showBackToHomePageBtn();
        return;
    }
...
pp.showBackToHomePageBtn = function () {
    ( this.$backBtn || $( "#back-btn" ) ).show();
}

pp.hideBackToHomePageBtn = function () {
    ( this.$backBtn || $( "#back-btn" ) ).hide();
}

pp.getDeviceWidth = function () {
    var i = 0,
        j = 0,
